...
<?php	endif; ?>
				<form class="form_standard" action="<?= $_SERVER['REQUEST_URI'] ?>" method="post" enctype="multipart/form-data">
					...
					<div class="div_input">
						<label for="input_player_ibfid" class="label_input">Id på innebandy.se:</label>
						<input name="data[ibfplayerid]" value="<?= ( ( $id > 0 ) ? $pdata['Player']['ibfplayerid'] : '' ) ?>" type="text" id="input_player_ibfid" />
					</div>					
<?php if ( $id > 0 && !empty($pdata['Player']['imgsrc']) ) : ?>
          <img class="admin_player_image" src="<?= Anchors::Refer('playerimage_folder') . '/' . $pdata['Player']['imgsrc']  ?>_tn.jpg" alt="<?= $pdata['Player']['firstname'] . ' ' . $pdata['Player']['lastname'] ?>" title="<?= $pdata['Player']['firstname'] . ' ' . $pdata['Player']['lastname'] ?>" />
          <a href="/admin/players/deleteimage/<?php echo $pdata['Player']['id']; ?>">ta bort bild</a>
<?php else: ?>
          <div class="div_input">
...
<?php	foreach ( $players as $k => $player ) : ?>
					<tr class="tr_player_row tr_<?= ( ( ($k+1)%2 ) ? 'odd' : 'even' ) ?>">
						...
						<td class="td_player_edit td_edit"><a href="<?= Anchors::Refer('admin_players') . '/edit/' . $player['Player']['id'] ?>">editera</a></td>						
