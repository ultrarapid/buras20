...
{	
	...
	private function ConvertToGametime($time, $period)
	{
		...
		return (int)substr($time, 0, 1) + ($period - 1) * 2 . str_replace('.', ':', substr($time, 1));
	}
  ...
  private function ExtractGameID($row)
	...
	private function GetGamePlayers($url)
	{
		$gamePlayers = array();
		/*
		if (  ( $gibfXml = $this->GetGibfXml("select * from html where url = '" . $url . "' AND xpath=\"//div/h2[text()='Matchhändelser']/following-sibling::table/tr/following::tr\"") ) !== false ) { }*/

		if (  ( $gibfXml = $this->GetGibfXml("select * from html where url = '" . $url . "' AND xpath=\"//div[@class='tableContainer']\"") ) !== false ) {
			$hometeam = $gibfXml->documentElement->getElementsByTagName('results')->item(0)->firstChild->firstChild->firstChild->getElementsByTagName('a')->item(0)->nodeValue;
			$awayteam = $gibfXml->documentElement->getElementsByTagName('results')->item(0)->firstChild->nextSibling->firstChild->firstChild->getElementsByTagName('a')->item(0)->nodeValue;

			$playerAnchors;

			if ( $hometeam == $this->team ) {
				$playerAnchors = $gibfXml->documentElement->getElementsByTagName('results')->item(0)->firstChild->firstChild->nextSibling->getElementsByTagName('a');
			} else if ( $awayteam == $this->team ) {
				$playerAnchors = $gibfXml->documentElement->getElementsByTagName('results')->item(0)->firstChild->nextSibling->firstChild->nextSibling->getElementsByTagName('a');
			}

			foreach ( $playerAnchors as $k => $pa) {
				$href = str_replace('%20', '', $pa->getAttribute('href'));
				//$gamePlayers[$k]['href'] = $href;
				$gamePlayers[$k]['id']   = substr($href, strpos($href, 'PlayerID=') + 9, strpos($href, "&epslanguage") - strpos($href, 'PlayerID=') - 9);
				$nameWithComma = $pa->nodeValue;
				$gamePlayers[$k]['name'] = substr($nameWithComma, strpos($nameWithComma, ', ') + 2) . ' ' . substr($nameWithComma, 0, strpos($nameWithComma, ', '));
			}
